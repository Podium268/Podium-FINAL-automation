...
module.exports = async function runRateAndMessageMultipleLadies(page, tierConfigs) {
  ...
  const excludedLadyNames = new Set([
    'Bella Swan','Veronica Park','smyle','Dee Dee Kelley','Indila','Zelda Hyrule','Her Majesty','Felis Felicitas','Wild Rose','Agent X','Giggles','Pania','Everest','RAMBØ XT','Katniss Everdeen','Queen Taylen',
    // add all names you want to exclude
  ].map(n => n.toLowerCase())); // convert to lowercase for case-insensitive matching

  const m1 = 'The stars on your profile aren’t enough to rate your beauty!⭐';
  const m2 = 'Stars can’t compete with your shine⭐'; //already won
  const m3 = 'Stars can’t compete with your shine⭐'; //168
