...
module.exports = async function runRateAndMessageMultipleLadies(page, tierConfigs) {
  ...
  const excludedLadyNames = new Set([
    'Bella Swan','Veronica Park','smyle','Dee Dee Kelley','Indila','Zelda Hyrule','Her Majesty','Felis Felicitas','Wild Rose','Agent X','Giggles','Pania','Everest','RAMBØ XT','Katniss Everdeen','Queen Taylen','.Auksė',
    // add all names you want to exclude
  ].map(n => n.toLowerCase())); // convert to lowercase for case-insensitive matching

  const m1 = 'Go on, eat the apple🍎, beauty.After that, let’s see whose name the mirror says.(Snow White reference)';
  const m2 = 'Go on, eat the apple🍎, beauty.After that, let’s see whose name the mirror says.(Snow White reference)'; //already won
  const m3 = 'Go on, eat the apple🍎, beauty.After that, let’s see whose name the mirror says.(Snow White reference)'; //168
